...
namespace CarPark
{
    public partial class FormCarPark : Form
    {
        ...
        {
            var queryEmptyBus = db.Bus.Where(bus => bus.Condition.Name == "На базе").OrderBy(bus => bus.Brand);
        ...
        {
            var queryInFlightBus = db.Bus.Where(bus => bus.Condition.Name == "В рейсе").OrderBy(bus => bus.Brand);
        ...
        {
            var queryUnderRepair = db.Bus.Where(bus => bus.Condition.Name == "В ремонте").OrderBy(bus => bus.Brand);
        ...
        {
            var queryDecommissioned = db.Bus.Where(bus => bus.Condition.Name == "Списан").OrderBy(bus => bus.Brand);
        ...
        {
            var all = db.Bus.Select(allBus => allBus).OrderBy(bus => bus.Brand);
